...
create view crimepercapita as
	select c.county, count(c.agency) as agencies, c.year, pop.population, sum(c.index_total)/pop.population as crimesperperson, sum(c.index_total), sum(c.violent_total)/pop.population as violentperperson, sum(c.violent_total), sum(c.murder)/pop.population as murderperperson, sum(c.murder), sum(c.rape)/pop.population as rapeperperson, sum(c.rape), sum(c.robbery)/pop.population as robberyperperson, sum(c.robbery), sum(c.aggravated_assault)/pop.population as aggravatedassaultperperson, sum(c.aggravated_assault), sum(c.property_total)/pop.population as propertyperperson, sum(c.property_total), sum(c.burglary)/pop.population as burglaryperperson, sum(c.burglary), sum(c.larceny)/pop.population as larcenyperperson, sum(c.larceny), sum(c.motor_vehicle_theft)/pop.population as vehicletheftperperson, sum(c.motor_vehicle_theft)
    from crimedata c, population pop
    where c.county = pop.county and c.year = pop.year
    group by c.county, c.year
    order by c.year, c.county;

select * from crimepercapita;
